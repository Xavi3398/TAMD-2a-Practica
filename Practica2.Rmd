...
Emprant ENN, tampoc no es noten canvis respecte de no rebalancejar.
...
df = df_resultats(to_dataframe(results), credit.test$Class, "Hybrid Resampling", "SMOTE+TK", models = c("tree", "knn"))
df.resultats = rbind(df.resultats, df)
df
```

En aquest cas, estam emprant totes les dades de la classe majoritària. Els resultats són molt pareguts al cas anterior, amb una mica millor "F1 Measure" i pitjor "Balanced Accuracy". 
...
Els resultats són quasi idèntics als aconseguits emprant SMOTE+TL.
...
df = df_resultats(to_dataframe(results), credit.test$Class, "Hybrid Resampling", "SMOTE+R.Under+TK", models = c("tree", "knn"))
df.resultats = rbind(df.resultats, df)
df
```

Els resultats obtinguts són prou bons, i molt pareguts als obtinguts amb el mètode de SMOTE amb random undersampling. Pareix que el fet d'aplicar després Tomek Links no canvia molt les coses.
...
# arbre = rpart(Class ~ ., data = credit.train)
# prediccio = predict(arbre, newdata = credit.test, probability = TRUE)
# df = df_metriques(prediccio, credit.train, credit.test$Class, "Cap", "Cap", "arbre", pq=FALSE)
# df
```
...
# prediccio2[prediccio == 1, 2] = prediccio_prob[prediccio == 1]
# prediccio2[prediccio == 1, 1] = 1 - prediccio_prob[prediccio == 1]
# df = df_metriques(prediccio2, credit.train, credit.test$Class, "Cap", "Cap", "knn", pq=FALSE)
# df
```
...
# svmfit = svm(Class ~ ., data = credit.train, kernel = "radial", cost = 1, scale = FALSE, probability = TRUE)
# prediccio = predict(svmfit, credit.test, probability = TRUE)
# df = df_metriques(attr(prediccio,"probabilities"), credit.train, credit.test$Class, "Cap", "Cap", "SVM", pq=FALSE)
# df
```
